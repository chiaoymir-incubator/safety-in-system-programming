    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                C.set(i+1, j+1, cmp::max(C.get(i+1, j).unwrap(), C.get(i, j+1).unwrap()));
            }
        }
    }
...
fn main() {
    ...
    let seq1 = read_file_lines(filename1).expect("cannot open file1");
    let seq2 = read_file_lines(filename2).expect("cannot open file2");
    let grid = lcs(&seq1, &seq2);
    print_diff(&grid, &seq1, &seq2, seq1.len(), seq2.len())
}
