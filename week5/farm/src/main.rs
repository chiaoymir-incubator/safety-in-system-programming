...
fn get_number(queue_ref: Arc<Mutex<VecDeque<u32>>>) -> Option<u32> {
...
fn main() {
    ...
    let num_len = input_numbers.len();
    let num_queue: Arc<Mutex<VecDeque<u32>>> = Arc::new(Mutex::new(input_numbers));

    // TODO: spawn `num_threads` threads, each of which pops numbers off the queue and calls
    // factor_number() until the queue is empty
    let mut threads = Vec::new();
    for _ in 0..num_len {
        let num_queue_ref = num_queue.clone();
        threads.push(thread::spawn(move || {
            if let Some(num) = get_number(num_queue_ref) {
                factor_number(num);
            }
        }));
    }
